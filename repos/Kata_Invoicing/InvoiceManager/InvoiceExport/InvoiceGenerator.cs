...
namespace Kata_Invoicing.InvoiceManager.InvoiceExport
{
    public class InvoiceGenerator
    {
        ...
        public List<SettlementInvoice> SettlementInvoice;
        public List<Refund> Refund;
        public List<RefundFee> RefundFee;
        ...
        public void ConvertExcelToPdf(string excelFilePath, string pdfFilePath)
        {
            ...
            using (var workbook = new XLWorkbook(excelFilePath))
            {
                ...
                // Craete a PDF Document
                using (var pdfDocument = new PdfDocument())
                {
                    pdfDocument.Info.Title = "Converted from Excel";
                    ...
                    foreach (var row in worksheet.RowsUsed())
                    {
                        ...
                        foreach (var cell in row.CellsUsed())
                        {
                            ...
                            graphics.DrawString(cellValue, font, XBrushes.Black, new XRect(xPoint, yPoint, pdfPage.Width, pdfPage.Height), XStringFormats.TopLeft);
                            xPoint += 100; // Adjust column spacing
                        }
                        yPoint += 20; // Adjust row spacing

                        // Create a new page if the height of the current page is exceeded
                        if (yPoint > pdfPage.Height - 20)
                        {
        ...
        private void GetInvoiceDataFromDb()
        {
            try
            {
                ...
                SettlementInvoice = InvoiceService.GetSettlementInvoices(invoiceId);
                Refund = InvoiceService.GetRefunds(invoiceId);
                RefundFee = InvoiceService.GetRefundFees(invoiceId);
